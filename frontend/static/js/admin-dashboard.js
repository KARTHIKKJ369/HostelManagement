...
function renderReportTable(headers, rows) {
    ...
    const thead = `<thead><tr>${headers.map(h => `<th>${escapeHTML(headerLabel(h))}</th>`).join('')}</tr></thead>`;
    const tbody = rows.length
        ? rows.map(r => `<tr>${headers.map(h => `<td>${escapeHTML(formatCell(h, r[h]))}</td>`).join('')}</tr>`).join('')
...
async function downloadReportPDF() {
    ...
    // For certain datasets, hide sensitive/noisy columns in PDF
    ({ headers, rows } = filterColumnsForPDF(entity, headers, rows));

        // Build summary using same helpers as preview
        const breakdowns = buildBreakdowns(headers, rows);
        const summaryHTML = renderSummaryTablesForPDF({
            entity,
            total: rows.length,
    ...
    const title = `Report: ${entity.charAt(0).toUpperCase() + entity.slice(1)} — ${new Date().toLocaleString()}`;
        ...
        const content = `
            <div class="report-root">
                <h1 style="font-size:18px; margin:0 0 4px 0;">${title}</h1>
                <div style="font-size:12px; color:#6b7280; margin:0 0 10px 0;">Generated by Admin Dashboard</div>
                <div class="report-summary">${summaryHTML}</div>
                ${note}
                <div class="report-table">${tableHTML}</div>
            </div>`;
    // Decide page settings based on columns
    const cols = headers.length;
    const landscape = cols > 8;
    const fontSize = cols > 12 ? 9 : (cols > 9 ? 10 : 11);
    printReport(content, title, { landscape, fontSize });
...
function printReport(innerHTML, docTitle = 'Report', options = {}) {
    ...
    if (!popup) {
        ...
        const fs = options.fontSize || 11;
        const cellPad = fs <= 9 ? '4px 6px' : '6px 8px';
        const styles = `
      <style>
    @page { size: ${pageSize}; margin: 12mm; }
                    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif; color: #111827; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        h1, h2, h3, h4 { color: #111827; }
            .table { width: 100%; border-collapse: collapse; font-size: ${fs}px; table-layout: fixed; }
            .table th, .table td { border: 1px solid #e5e7eb; padding: ${cellPad}; vertical-align: top; word-break: break-word; overflow-wrap: anywhere; white-space: normal; }
    /* Allow header to wrap across words to avoid overlap */
    .table thead th { background: #f3f4f6; text-align: left; white-space: normal; word-break: keep-all; overflow-wrap: anywhere; }
                thead { display: table-header-group; }
                tfoot { display: table-footer-group; }
                tr { page-break-inside: avoid; }
                    .form-row, .form-group, .report-summary, .report-table { break-inside: avoid; page-break-inside: avoid; }
    ...
    setTimeout(() => { popup.print(); popup.close(); }, 350);
}
...
function renderReportTableForPDF(headers, rows) {
    ...
    const colgroup = buildColGroup(headers);
    const thead = `<thead><tr>${headers.map(h => `<th>${escapeHTML(headerLabel(h))}</th>`).join('')}</tr></thead>`;
    const tbody = rows && rows.length
        ? rows.map(r => `<tr>${headers.map(h => `<td>${escapeHTML(formatCell(h, r[h]))}</td>`).join('')}</tr>`).join('')
        : `<tr><td colspan="${headers.length}" style="text-align:center; color:#6b7280;">No rows</td></tr>`;
    return `<table class="table">${colgroup}${thead}<tbody>${tbody}</tbody></table>`;
}
...
// Pretty-print common date/time fields for readability in PDF/preview
function formatCell(header, value) {
    if (value === undefined || value === null) return '';
    const h = String(header).toLowerCase();
    const asString = String(value);
    // Detect typical date/time columns
    const isDateLike = /(_at$|_date$|date$|time$)/.test(h);
    if (!isDateLike) return asString;
    // Try to parse
    const d = new Date(asString);
    if (isNaN(d.getTime())) return asString; // fallback if unparsable
    ...
    // If the column is a pure date (like due_date or allotment_date), omit time
    const isPureDate = /(_date$|^date$)/.test(h) && !/_at$/.test(h);
    return isPureDate ? `${yyyy}-${mm}-${dd}` : `${yyyy}-${mm}-${dd} ${hh}:${mi}:${ss}`;
}

// Hide/keep certain columns by entity for PDF readability
function filterColumnsForPDF(entity, headers, rows) {
        const ent = String(entity || '').toLowerCase();
        let keep = headers.slice();
        if (ent === 'applications') {
                // Keep a focused set for Applications PDFs
                const preferred = [
                        'id','application_id','user_id','preferred_hostel_id','room_type_preference',
                        'course','academic_year','performance_type','performance_value','distance_from_home','distance_unit',
                        'status','reviewed_by','reviewed_at','allocated_room_id','allocated_at','created_at','updated_at'
                ];
                keep = headers.filter(h => preferred.includes(String(h)));
                if (!keep.length) keep = headers; // fallback
        }
        const idx = new Map(keep.map((h,i)=>[String(h), i]));
        const outHeaders = keep;
...
// Render a compact summary header (as tables) for PDFs
function renderSummaryTablesForPDF({ entity, total, generatedAt, breakdowns }) {
        const dt = generatedAt instanceof Date ? generatedAt : new Date();
        const metaTable = `
                <table class="table" style="margin:6px 0 10px 0; width:100%;">
                    <tbody>
                        <tr>
                            <td style="width:40%;"><strong>Dataset</strong></td><td>${headerLabel(entity)}</td>
                            <td style="width:20%;"><strong>Total rows</strong></td><td>${Number(total||0).toLocaleString()}</td>
                        </tr>
                        <tr>
                            <td><strong>Generated</strong></td><td>${dt.toLocaleString()}</td>
                            <td><strong>Source</strong></td><td>Admin Dashboard</td>
                        </tr>
                    </tbody>
                </table>`;
        // Top two breakdowns (if any), side-by-side
        const entries = Object.entries(breakdowns||{}).slice(0,2);
        let bhtml = '';
        if (entries.length) {
                const [a,b] = entries;
                const renderOne = ([field, list]) => {
                        const rows = (list||[]).map(([k,v]) => `<tr><td>${escapeHTML(String(k))}</td><td style="text-align:right;">${v}</td></tr>`).join('') || '<tr><td colspan="2" style="color:#6b7280;">No data</td></tr>';
                        return `<table class="table" style="width:100%;"><thead><tr><th colspan="2">${escapeHTML(headerLabel(field))}</th></tr></thead><tbody>${rows}</tbody></table>`;
                };
                bhtml = `<div style="display:grid; grid-template-columns: repeat(${entries.length}, 1fr); gap:8px; margin-bottom:10px;">${[a,b].filter(Boolean).map(renderOne).join('')}</div>`;
        }
        return `${metaTable}${bhtml}`;
}

// Make column headers nicer for humans
...
// Assign relative widths to columns and build a <colgroup>
function buildColGroup(headers) {
    const isId = h => /(^|_)id$/.test(h);
    const isDate = h => /(_at$|_date$|^date$|time)/.test(h);
    const isText = h => /(description|message|notes|address|name|title)/.test(h);
    const isNumber = h => /(amount|total|fee|paid|capacity|rank|sgpa|score|deposit)/.test(h);
    const isLongHeader = h => String(h).length > 16;
    const weights = headers.map(h => {
        const hh = String(h).toLowerCase();
        if (isText(hh)) return 2.3;
        if (isId(hh)) return 0.8;
        if (isDate(hh)) return 1.2;
        if (isNumber(hh)) return 1.1;
        return isLongHeader(h) ? 1.3 : 1.0;
    });
    const sum = weights.reduce((a,b)=>a+b,0) || 1;
    // Base percentages as floats
    let pcts = weights.map(w => (w / sum) * 100);
    // Minimum per-column percentage: smaller for many columns
    const minP = Math.max(3, Math.floor(100 / (headers.length * 1.6)));
    pcts = pcts.map(p => Math.max(minP, p));
    // Normalize to 100%
    const total = pcts.reduce((a,b)=>a+b,0) || 1;
    const factor = 100 / total;
    pcts = pcts.map(p => p * factor);
    const colEls = pcts.map(p => `<col style="width:${p.toFixed(2)}%">`).join('');
    return `<colgroup>${colEls}</colgroup>`;
}
...
async function systemLogs() {
    ...
    if (ent === 'users') {
        // Hide sensitive and overly long columns from Users PDFs
        const excluded = new Set(['password_hash']);
        keep = headers.filter(h => !excluded.has(String(h)));
    }
}
